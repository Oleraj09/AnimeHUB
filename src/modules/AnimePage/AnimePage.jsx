...
const AnimePage = () => {
  ...
  const urls = () => {
    if (page === "upcoming-anime") {
      return `https://api.jikan.moe/v4/seasons/upcoming?page=${pageNumber}`;
    }
    if (page === "top-anime") {
      return `https://api.jikan.moe/v4/top/anime?page=${pageNumber}`;
    }
    if (page === "season-now") {
      return `https://api.jikan.moe/v4/seasons/now?page=${pageNumber}`;
    }
    if (page === "airing-now") {
      return `https://api.jikan.moe/v4/schedules/${dayName}?page=${pageNumber}`;
    }
    if (page === "scheduled") {
      return `https://api.jikan.moe/v4/schedules?page=${pageNumber}`;
    }
    if (page === "anime-list") {
      return `https://api.jikan.moe/v4/anime?page=${pageNumber}`;
    }
    return "";
  };
  ...
  return (
    <>
      <div className="anime-page select-none">
        <Breadcrumbs />
        {data?.data?.length > 0 ? (
          <>
            ...
            {data?.pagination && (
              <div className="flex justify-center items-center gap-4 mt-6 mb-12">
                <button
                  ...
                  className={`px-4 py-2 flex items-center h-[40px] rounded-full w-[40px] ${pageNumber <= 1 ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue cursor-pointer'}`}
                >
                  <FontAwesomeIcon icon={faAngleLeft}></FontAwesomeIcon>
                </button>

                <span className="text-[16px] text-gray-600">
                  Page {pageNumber} of {data.pagination.last_visible_page}
                </span>
